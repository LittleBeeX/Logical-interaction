...
<script type="text/javascript">
	...
	$('#pass').on('click',function(){
		...
		tips('','','0',{
			...
			cb: function() {
				$.ajax({
					url : '<?=$this->url("ltd_bvi/member_pass")?>',
					type : 'POST',
					data : {oid:oid,code:code},
					dataType : "json",
			        success:function(e){
			        	if (e.state == 0) {
							window.location.href="<?=$this->url("ltd_bvi/member_list")?>?oid="+oid+"&code="+code;
			     			return false;
			        	}
	...
	$('#btn').on('click',function(){
		...
		tips('','','0',{
			...
			cb: function() {
				$.ajax({
					url : '<?=$this->url("ltd_bvi/people_ajax")?>',
					type : 'POST',
					data : data,
					dataType : "json",
			        success:function(e){
			        	if (e.state == 0) {
			        		...
			        		setTimeout(function(){
								window.location.href="<?=$this->url("ltd_bvi/member_list")?>?oid="+e.oid+"&code="+e.code+'&is_pay='+is_pay;
			        		},2000)
			        		return false;
			        	}
			      		tips(e.info,'warning',1000);
	...
	$(".upload").change(function(){
		...
		tips('','','0',{
			...
			cb: function() {
				$.ajax({
		            url:'<?= $this->url('ltd_bvi/hz_upload_ajax')?>',
		            type:'POST',
		            data:formData,
		            cache: false,
