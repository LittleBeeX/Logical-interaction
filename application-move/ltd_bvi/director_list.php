<div class="pt110 page-edit">
	...
	<ul class="user-info-list plr30 fz24 cff">
		...
		<?foreach ($people as $key => $val) {?>
			<li class="ub ub-ac">
				...
				<?if (empty($is_pay)) {?>
					<a href="<?= $this->url('ltd_bvi/add_director')?>?oid=<?= $val['oid']?>&id=<?= $val['id']?>&code=<?=$code?>&noback=1" class="user-info-edite ub ub-pac cff"><?= $director_list['upd']?></a>
					<div data-id="<?= $val['id']?>" class="user-info-del ub ub-pac del"><?= $director_list['del']?></div>
				<?}else{?>
					<?if ($val['audit_state'] == 2 || $val['audit_state'] == 1) {?>
						<a href="<?= $this->url('ltd_bvi/add_director')?>?id=<?= $val['id']?>&code=<?=$code?>&is_pay=<?= $is_pay?>&noback=1" class="user-info-edite ub ub-pac cff"><?= $director_list['look']?></a>
					<?}else{?>
						<a href="<?= $this->url('ltd_bvi/add_director')?>?oid=<?= $val['oid']?>&id=<?= $val['id']?>&code=<?=$code?>&noback=1&is_pay=<?= $is_pay?>" class="user-info-del ub ub-pac cff"><?= $director_list['upd']?></a>
					<?}?>
				<?}?>
			</li>
		<?}?>
	...
	<?if (empty($is_pay)) {?>
		<a href="<?= $this->url('ltd_bvi/add_director')?>?oid=<?= $oid?>&code=<?= $code?>&noback=1" class="ub ub-pac bgf76959 fz26 cff lh1 ptb40 mb40 mt30 add-directors"><b class="fz32 fw-b pr10">+</b><?= $director_list['new']?></a>
		<?if (empty($code)) {?>
			<a href="javascript:void(0)" id="next" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30"><?= $director_list['next']?></a>
		<?}else{?>
			<a href="<?=$this->url('ltd_bvi/order_show')?>?oid=<?= $oid?>" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30"><?= $director_list['con']?></a>
		<?}?>	
	<?}else{?>
		<a href="<?=$this->url('ltd_bvi/order_show')?>?oid=<?= $oid?>" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30"><?= $director_list['back']?></a>
	<?}?>
				...
				$.ajax({
			        ...
			        success:function(e){
			     		if (e.state == 0) {
			     			/*code NULL 正常流程*/
			     			if (e.code == 0) {
			     				window.location.href="<?= $this->url('ltd_bvi/add_member')?>?oid="+oid+'&code='+code;return false;
			     			}else{
			     			/*code true 去个人中心*/
