 ...
 * @param unknown $menu         
 */
function get_menu_tree_hx($menu = array(), $selected = array(), $field = 'menuid') {
  foreach ( $menu as $key => $value ) {
    echo '<li id="' . $value [$field] . '" ' . (! empty ( $selected ) && in_array ( $value ['id'], $selected ) ? 'class="selected"' : "") . '>' . $value ['title'];
    if (isset ( $value ['childs'] ) && is_array ( $value ['childs'] )) {
      echo '<ul>';
      get_menu_tree_hx ( $value ['childs'], $selected, $field );
      echo '</ul></li>';
    } else {
      echo '</li>';
    }
