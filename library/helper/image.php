...
class image {
     ...
     * return volid
     */
    function save($filename,$image_type='') {
        if(empty($image_type)){
     ...
     * @param int $height 指定高度
     */
    function cutByCenter($width,$height){
    ...
    function vcut($top,$height){
        ...
        imagecopy($newImage, $this->image, 0, 0, 0, $top, $width, $height);
        imagedestroy($this->image);
        $this->image = $newImage;
    }
    ...
    */
    ...
    /**
    * 保存缩略图 按照比例 从图片中心取缩略图
    */
    function createThumb($maxWidth=400,$maxHeight=300,$ext='_thumb'){
        $width  = $this->getWidth();
        $height = $this->getHeight();
        
            
        // create thumb
        $thumbFilePath = getThumbPath($this->fileName,$ext);            
             
        if($width/$height == $maxWidth/$maxHeight){
            // 等比缩放
            $this->resizeScale($maxWidth , $maxHeight);
        } else {
            if($width >= $height){
                $newWidth = ($maxWidth/$maxHeight) * $height;
                $this->cutByCenter($newWidth,$height);
            } else {
                $newHeight = ($maxHeight/$maxWidth) * $width;
                $this->cutByCenter($width,$newHeight);
            }
            //$this->resizeScale($maxWidth , $maxHeight);
            //$this->cutByCenter($maxWidth,$maxHeight);
        }
        
        // save thumb file.
        $this->save($thumbFilePath , IMAGETYPE_JPEG);
