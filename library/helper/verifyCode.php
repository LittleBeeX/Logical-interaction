  ...
  class verifyCode{ 
    ...
    function createZF(){                    
        if($this->type == 3){
            ...
            for($i=0;$i<$this->num;$i++){
                imagechar($this->img,8,$i*($this->width/$this->num),rand(12,$this->height-15),$this->string[$i],$this->Scolor()); 
            }
        } 
    } 
    function getString(){ 
        switch($this->type){//根据选择的类型得到相应的字符串 
        case 0:
            $this->string=array_rand(range(1,9),$this->num); 
            break; 
        case 1: 
            ...
            break; 
        case 2: 
            $arr1=array_merge(range(0,9),range('a','z'),range('A','Z'));
            $arr2 = array('0','1','o','O','l');
            $arr=array_merge(array_diff($arr1,$arr2));
            $this->string=array_rand(array_flip($arr),$this->num);  
            break;             
        case 3:
...
"月 ","星");
    ...
    function createGRYS(){ 
        /*for($i=0;$i<4;$i++) imagearc($this->img,rand(0,$this->width),rand(0,$this->height),rand(20,$this->width/1.5),rand(2,8),rand(0,10),rand(80,120),$this->Scolor());
        for($i=0;$i<50;$i++) imagesetpixel($this->img,rand(0,$this->width),rand(0,$this->height),$this->Scolor());*/
       // for($i=0;$i<4;$i++) {
          //  imagearc($this->img,$this->width/2,$this->height/2,$this->width,rand(10,70),50,50,$this->Scolor());
       // }
	   for($i=0;$i<2;$i++) {
	  imagearc($this->img,rand(30,$this->width),rand(0,$this->height),$this->width,rand(2,3),$this->width,$this->height,$this->Scolor());
	   }
    } 
    function createBj(){
    ...
    function Qcolor(){//生成一个浅颜色 
        return imagecolorallocate($this->img,rand(255,255),rand(255,255),rand(255,255));
        //return imagecolorallocate($this->img,rand(150,255),rand(150,255),rand(150,255)); 
    } 
    function Scolor(){//生成一个深颜色 
        return imagecolorallocate($this->img,rand(10,30),rand(90,100),rand(150,160)); 
    }
    function createHB(){ 
