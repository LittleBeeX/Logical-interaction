		...
		<li>
			<div class="ubi ub-ac apply-reg-hint">
				...
				<div class="selectbox ub-f1">
					...
					<?foreach ($company_cym_lastname as $key_a => $value_a) {?>
...
<?if (empty($is_pay)) {?>
	...
	<?}?>
<?}else{?>
	<a href="<?=$this->url('ltd_cym/order_show')?>?oid=<?= $oid?>" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30"><?= $basic['back']?></a>
<?}?>
						...
						<div class="apply-slide-item pt40 mt20">
							...
							<div class="apply-slide-con fz22 lh16 word-break pt10">
								<div class="apply-flow pb40">
									<img src="/public/assets/weixin/images/ltd_cym_<?=$this->languageName?>.png" alt="<?= $guide['nav1']['div4']?>" />
								</div>
							</div>
						</div>
					</div>
...
$('#btn').on('click',function(){
	...
	tips('','','0',{
		...
		cb: function() {
			$.ajax({
				url : '<?=$this->url("ltd_cym/basic_ajax")?>',
				type : 'POST',
				data : data,
				dataType : "json",
		        success:function(e){
		        	if (e.state == 0) {
		        		...
		        		setTimeout(function(){
		        			if (e.code == '') {
		        				window.location.href="<?=$this->url("ltd_cym/project")?>?oid="+e.oid+'&code='+'';
		        			}else{
