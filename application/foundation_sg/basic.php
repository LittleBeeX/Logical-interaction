...
<?if (empty($is_pay)) {?>
	...
	<?}?>
<?}else{?>
	<a href="<?=$this->url('foundation_sg/order_show')?>?oid=<?= $oid?>" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30 "><?= $basic['back']?></a>
<?}?>
			...
			<div class="apply-slide-main ub-f1 w100">
				<div class="bs-wrapper apply-wrapper">
						...
						<div class="apply-slide-item pt40 mt20">
							...
							<div class="apply-slide-con fz22 lh16 word-break pt10">
								<div class="apply-flow pb40">
									<img src="/public/assets/weixin/images/foundation_sg_<?=$this->languageName?>.png" alt="<?= $guide['nav1']['div4']?>" />
								</div>
							</div>
						</div>
					</div>
					...
					<div class="bs-slide apply-slide plr30 pb40 swiper-slide-active" style="width: 375px;">
						...
						<div class="apply-slide-item pt40 mt20">
							<div class="apply-slide-tit ub ub-tc fz28 fw-b">
								<i class="iconfont icon-start fz24 pr10"></i><?= $guide['nav3']['div6']?></div>
							<div class="apply-slide-con fz22 pl30 lh16 word-break pt10">
								<p><?= $guide['nav3']['p6']?></p>
							</div>
						</div>
...
$('#btn').on('click',function(){
	...
	tips('','','0',{
		...
		cb: function() {
			$.ajax({
				url : '<?=$this->url("foundation_sg/basic_ajax")?>',
				type : 'POST',
				data : data,
				dataType : "json",
		        success:function(e){
		        	if (e.state == 0) {
		        		...
		        		setTimeout(function(){
		        			if (e.code == '') {
		        				window.location.href="<?=$this->url("foundation_sg/project")?>?oid="+e.oid+'&code='+'';
		        			}else{
		        				window.location.href="<?=$this->url("foundation_sg/order_show")?>?oid="+e.oid;
		        			}
		        		},2000)
		        		return false;
		        	}
