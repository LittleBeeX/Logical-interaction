<div class="pt110">
	...
	<?}else{?>
		...
		<?}else{?>
			<a href="<?=$this->url('foundation_sg/order_show')?>?oid=<?= $oid?>" class="ub ub-pac bg0070ba fz26 cff lh1 ptb40 mb40 mt30"><?= $projects['back']?></a>
		<?}?>
	<?}?>
...
<script>
	...
	$('#btn').on('click',function(){
		...
		cb: function() {
			   $.ajax({
	        	url : '<?=$this->url("foundation_sg/project_ajax")?>?oid='+oid+'&code='+code,
				type : 'POST',
		        ...
		        success:function(e){
			        if(e.state == 0){
		        		...
		        		setTimeout(function(){
		        			if (e.code == '') {
		        				window.location.href="<?=$this->url("foundation_sg/add_director")?>?oid="+e.oid+'&code='+e.code;
		        			}else{
