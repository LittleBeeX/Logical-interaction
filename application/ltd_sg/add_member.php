...
<script type="text/javascript">
	...
	$('#pass').on('click',function(){
		...
		tips('','','0',{
			...
			cb: function() {
				$.ajax({
					url : '<?=$this->url("ltd_sg/member_pass")?>',
					type : 'POST',
					data : {oid:oid,code:code},
					dataType : "json",
			        success:function(e){
			        	if (e.state == 0) {
							window.location.href="<?=$this->url("ltd_sg/member_list")?>?oid="+oid+"&code="+code;
			     			return false;
			        	}
				...
				$.ajax({
			        ...
			        success:function(e){
			        	if (e.state == 0) {
			        		...
			        		setTimeout(function(){
								window.location.href="<?=$this->url("ltd_sg/member_list")?>?oid="+e.oid+"&code="+e.code+'&is_pay='+is_pay;
			        		},2000)
			        		return false;
			        	}
			      		tips(e.info,'warning',1000);
